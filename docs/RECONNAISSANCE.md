...
| Somnia **CLI** (`somnia-dex-cli`), **ccxt fork**, `AGENTS.md`/`SKILL.md` | Alternative integrations | — | — | Justified non-use | TEMPO is a CLI/SDK; using the venue's markets SDK directly is deeper than a wrapper-of-a-wrapper |
| Model Context Protocol | Structured autonomous-agent interoperability | bounded tool schemas and stdio transport | External agents can inspect Tempo state and request policy-gated action | **Yes** | `@tempo/mcp` exposes live reads, dry-run simulation, and an opt-in Executor-backed write tool |
